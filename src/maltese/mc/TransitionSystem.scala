...
case class TransitionSystem(name: String, inputs: List[BVSymbol], states: List[SMTSymbol], init: List[Signal], next: List[Signal]) {
  def serialize: String = TransitionSystem.serialize(this)
  def signals: Iterable[Signal] = init ++ next
}
...
object TransitionSystem {
  def serialize(sys: TransitionSystem): String = {
    (Iterator(sys.name) ++
      sys.inputs.map(i => s"input ${serialize(i)}") ++
      sys.states.map(s => s"state ${serialize(s)}") ++
      (if(sys.init.nonEmpty) Seq("init:") ++ sys.init.map(serialize) else Seq()) ++
      (if(sys.next.nonEmpty) Seq("next:") ++ sys.next.map(serialize) else Seq())
      ).mkString("\n")
  }

  def serialize(s: Signal): String =
    s"${SignalLabel.labelToString(s.lbl)} ${s.name} : ${SMTExpr.serializeType(s.e)} = ${s.e}"
  def serialize(s: SMTSymbol): String = s"${s.name} : ${SMTExpr.serializeType(s)}"

  /** prefixes all signal names with the name of the transition system */
  def prefixSignals(sys: TransitionSystem): TransitionSystem = {
    val prefix = sys.name + "."
    val names: Iterable[String] = sys.inputs.map(_.name) ++ sys.states.map(_.name) ++ sys.signals.map(_.name)
    ...
    val states = sys.states.map(s => r(s).asInstanceOf[SMTSymbol])
    val init = sys.init.map(s => s.copy(name = renames(s.name), e = r(s.e)))
    val next = sys.next.map(s => s.copy(name = renames(s.name), e = r(s.e)))
    TransitionSystem(sys.name, inputs, states, init, next)
  }
  ...
  def combine(name: String, sys: List[TransitionSystem]): TransitionSystem = {
    TransitionSystem(name,
      ...
      init = sys.flatMap(_.init),
      next = sys.flatMap(_.next)
    )
  }
  ...
  def connect(sys: TransitionSystem, cons: Map[String, BVExpr]): TransitionSystem = {
    ...
    sys.copy(inputs = inputs, next = connections ++ sys.next)
  }
  ...
  def connectStates(sys: TransitionSystem, cons: Map[String, SMTExpr]): TransitionSystem = {
    ...
    val connections = cons.map(c => mc.Signal(c._1, c._2)).toList
    sys.copy(states = states, next = connections ++ sys.next)
  }

  def systemExpressions(sys: TransitionSystem): Iterable[smt.SMTExpr] = sys.signals.map(_.e)
